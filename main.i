...
void goToStart() {
    ...
    copyToCharBlock(titleScreenTiles, 0, 65536 >> 1);
    copyToScreenBlock(titleScreenMap, 24, 2048 >> 1);
    hideSprites();
    copyShadowOAM();
    stopSound();
    playSoundA(titleSong_data, titleSong_length, 1);
    state = START;
}
...
void goToInstructions() {
    ...
    copyToCharBlock(instructionScreenTiles, 0, 65536 >> 1);
    ...
    for (int i = 0; i < 4; i++) {
        switch(i) {
            case 0:
                ...
                instructionSprites[i].numFrames = 3;
...
void drawInstructions() {
    for (int i = 0; i < 4; i++) {
        switch(i) {
            case 0:
                shadowOAM[i].attr0 = (0xFF & (instructionSprites[i].worldRow)) | (0 << 14) | (0 << 13) | (0 << 8);
                shadowOAM[i].attr1 = (0x1FF & (instructionSprites[i].worldCol)) | (1 << 14);
                shadowOAM[i].attr2 = ((instructionSprites[i].curFrame * 2)*32 + (instructionSprites[i].aniState * 2)) | ((0) << 10) | ((0) << 12);
            break;
            case 1:
                shadowOAM[i].attr0 = (0xFF & (instructionSprites[i].worldRow)) | (0 << 14) | (0 << 13) | (0 << 8);
                shadowOAM[i].attr1 = (0x1FF & (instructionSprites[i].worldCol)) | (1 << 14);
            ...
            case 3:
                shadowOAM[i].attr0 = (0xFF & (instructionSprites[i].worldRow)) | (0 << 14) | (0 << 13) | (0 << 8);
                shadowOAM[i].attr1 = (0x1FF & (instructionSprites[i].worldCol)) | (0 << 14);
                shadowOAM[i].attr2 = ((0)*32 + (29)) | ((0) << 10) | ((0) << 12);
            break;
        }
    }
}
...
void goToPause() {
    (*(volatile unsigned short *)0x04000010) = 0;
    (*(volatile unsigned short *)0x04000012) = 0;
    playSoundB(pauseSound_data, pauseSound_length, 0);

    (*(volatile unsigned short *)0x4000008) = ((0) << 2) | (1 << 7) | (0 << 14) | ((24) << 8);
    copyToBGPaletteMem(pauseScreenPal, 512 >> 1);
    copyToCharBlock(pauseScreenTiles, 0, 65536 >> 1);
...
void goToWin() {
    ...
    copyToCharBlock(winScreenTiles, 0, 65536 >> 1);
...
void goToLose() {
    ...
    copyToCharBlock(loseScreenTiles, 0, 65536 >> 1);
